...
const TOKEN_FILE = path.join(process.cwd(), "data", "amazon-oauth.json");
...
async function writeTokenToSupabase(refreshToken: string): Promise<boolean> {
  ...
  if (!hasSupabaseServiceRoleKey()) {
    console.error(
      "Cannot save Amazon token: SUPABASE_SERVICE_ROLE_KEY is missing.",
    );
    return false;
  }
  ...
  try {
    const supabase = createSupabaseAdmin();
    const { error } = await supabase.from("amazon_oauth").upsert(
      {
        id: TOKEN_ROW_ID,
    ...
    if (error) {
      console.error("Failed to save Amazon token to Supabase:", error.message);
      return false;
    }
    ...
    console.error(
      "Failed to save Amazon token to Supabase:",
...
export async function getStoredAmazonRefreshToken(): Promise<string | null> {
...
export async function saveAmazonRefreshToken(
  refreshToken: string,
): Promise<void> {
  ...
  throw new Error(
    process.env.VERCEL
      ? hasSupabaseServiceRoleKey()
        ? "Could not store Amazon refresh token in Supabase. Check Supabase logs and redeploy."
        : "Add SUPABASE_SERVICE_ROLE_KEY to Vercel (Supabase → Settings → API → service_role), then redeploy and connect again."
      : hasSupabaseServiceRoleKey()
        ? "Could not store Amazon refresh token in Supabase."
        : "Add SUPABASE_SERVICE_ROLE_KEY to .env.local, or set AMAZON_REFRESH_TOKEN manually.",
  );
}

export async function clearStoredAmazonRefreshToken(): Promise<void> {
  if (isSupabaseConfigured()) {
    try {
      const supabase = createSupabaseAdmin();
      await supabase.from("amazon_oauth").delete().eq("id", TOKEN_ROW_ID);
    } catch {
      // ignore
    }
  }
